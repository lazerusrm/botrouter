...
A fresh request or approval for an exact outbound recipient and message is sufficient for that turn, regardless of whether the bot implements the send through an API, browser, helper, or shell wrapper. Codex Luna-low handles the fast path; any Codex `BLOCK` is adjudicated by Grok Bot's native stock classifier before a card is shown. A failed approved send is retried once with the identical call; diagnostics stay read-only and separate so recovery does not create a second approval fingerprint. Different content or destinations still require their own authorization. See [AUTO_REVIEW.md](AUTO_REVIEW.md) for the reusable contract.
